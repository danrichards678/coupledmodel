...
def IBOF_closure(a):
    ...
    Parameters
    ----------
    a : npx3x3 numpy array
        Second order fiber orientation tensor.

    Returns
    -------
    npx3x3x3x3 numpy array
        Fourth order fiber orientation tensor.
    ...
    delta = np.stack([np.eye(3)]*n)

    # generate fourth order tensor with parameters and tensor algebra
    return (
        symm(np.einsum("p,pij,pkl->pijkl", beta1, delta, delta))
        + symm(np.einsum("p,pij,pkl->pijkl", beta2, delta, a))
        + symm(np.einsum("p,pij,pkl->pijkl",beta3, a, a))
        + symm(np.einsum("p,pij,pkm,pml->pijkl", beta4, delta, a, a))
        + symm(np.einsum("p,pij,pkm,pml->pijkl", beta5, a, a, a))
        + symm(np.einsum("p,pim,pmj,pkn,pnl->pijkl", beta6, a, a, a, a))
    )
